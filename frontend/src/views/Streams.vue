  ...
  <div>
    ...
    <div class="bg-white rounded-t-xl p-5 border-b border-gray-200 shadow-sm overflow-hidden relative">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div class="flex items-center gap-3">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Streams</h2>
                    <p class="text-sm text-gray-500 mt-1">TCP/UDP proxy for backend services</p>
                </div>
            </div>
            <div class="flex items-center gap-3 w-full lg:w-auto">
                <div class="relative flex-1 lg:flex-initial">
                    <input 
                        ...
                        class="w-full lg:w-64 pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500 transition-all bg-gray-50/50"
                    />
    ...
    <div class="bg-white rounded-b-xl shadow-lg border-x border-b border-gray-200">
        ...
        <div v-if="streams.length === 0" class="p-12 text-center">
            ...
            <p class="text-lg font-bold text-gray-700 mb-2">No streams configured</p>
        ...
        <div v-else-if="filteredStreams.length === 0" class="p-12 text-center">
            ...
            <p class="text-sm text-gray-400">No streams match "{{ searchQuery }}"</p>
        </div>
        ...
        <template v-else>
            ...
            <div v-for="stream in filteredStreams" :key="stream.id" class="hidden lg:grid lg:grid-cols-12 gap-4 p-4 border-b border-gray-100 hover:bg-gray-50/50 transition-all duration-200 last:border-b-0 group items-center">
                ...
                <div class="col-span-3 flex items-center gap-3">
                    ...
                    <span class="text-sm font-bold text-gray-900">{{ stream.name || 'Unnamed Stream' }}</span>
                </div>
                
                <!-- Listen Port -->
                <div class="col-span-2">
                    <span class="font-mono bg-gray-100 px-3 py-1.5 rounded-lg text-gray-700 text-sm font-bold border border-gray-200">:{{ stream.listen_port }}</span>
                </div>
                ...
                <div class="col-span-3">
                    <span class="font-mono bg-gray-50 px-2 py-1 rounded-lg text-gray-600 text-xs border border-gray-200 break-all">{{ stream.target }}</span>
                </div>
                ...
                <div class="col-span-2 flex items-center justify-end gap-3">
                    <button @click="toggleStatus(stream)" :class="stream.is_active ? 'bg-green-500' : 'bg-gray-300'" class="relative inline-flex h-6 w-11 items-center rounded-full transition-all focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2">
                        <span :class="stream.is_active ? 'translate-x-6' : 'translate-x-1'" class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform shadow-sm" />
                    </button>
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-base font-bold text-gray-900 truncate">{{ stream.name || 'Unnamed Stream' }}</p>
                                <div class="flex items-center gap-2 mt-1">
                                    <span class="font-mono bg-gray-100 px-2 py-0.5 rounded text-gray-700 text-xs font-bold">:{{ stream.listen_port }}</span>
                                    <span class="text-gray-300">→</span>
                                    <span class="font-mono text-gray-500 text-xs truncate">{{ stream.target }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap gap-2 mb-4">
                            ...
                            <div class="flex items-center gap-2 ml-auto">
                                <span :class="stream.is_active ? 'text-green-600' : 'text-gray-400'" class="text-xs font-bold">{{ stream.is_active ? 'Active' : 'Inactive' }}</span>
                        ...
                        <div class="flex gap-2 pt-3 border-t border-gray-100">
                            ...
                            <button @click="deleteStream(stream.id)" class="flex items-center justify-center gap-2 px-4 py-2.5 border border-gray-200 text-red-500 hover:bg-red-50 hover:border-red-200 rounded-lg text-sm font-semibold transition-all">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                </svg>
                            </button>
      ...
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg transform transition-all overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div class="px-6 pt-4 flex-shrink-0">
            <div class="flex gap-1 p-1 bg-gray-100 rounded-xl">
                <button @click="activeTab = 'details'" :class="activeTab === 'details' ? 'bg-white shadow-sm text-yellow-600' : 'text-gray-600 hover:text-gray-800'" class="flex-1 py-2 px-3 rounded-lg font-medium text-sm transition-all">
                    Details
                </button>
                <button @click="activeTab = 'ssl'" :class="activeTab === 'ssl' ? 'bg-white shadow-sm text-yellow-600' : 'text-gray-600 hover:text-gray-800'" class="flex-1 py-2 px-3 rounded-lg font-medium text-sm transition-all">
                    SSL/TLS
                </button>
            </div>
        </div>
        ...
        <div class="p-6 space-y-5 overflow-y-auto flex-1">
            ...
            <div v-if="activeTab === 'details'" class="space-y-5">
                <div>
                    <span class="block text-sm font-semibold text-gray-700 mb-2">Name <span class="text-red-500">*</span></span>
                    <input v-model="newStream.name" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-gray-700 bg-gray-50/50 focus:outline-none focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500 focus:bg-white transition-all" placeholder="MySQL Database">
                </div>

                <div>
                    <span class="block text-sm font-semibold text-gray-700 mb-2">Listen Port <span class="text-red-500">*</span></span>
                    <input v-model.number="newStream.listen_port" type="number" min="1" max="65535" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-gray-700 bg-gray-50/50 focus:outline-none focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500 focus:bg-white transition-all" placeholder="3306">
                </div>

                <!-- Protocols Section -->
                <div class="border border-gray-200 rounded-xl overflow-hidden">
                    <div class="bg-gray-50 px-4 py-3 border-b border-gray-200">
                        <span class="font-semibold text-gray-700">Protocols</span>
                    </div>
                    <div class="divide-y divide-gray-100">
                        <div class="flex items-center justify-between px-4 py-3 hover:bg-gray-50/50 transition-colors">
                            <div class="flex items-center gap-2">
                                <span class="bg-purple-100 text-purple-700 px-2 py-0.5 rounded text-xs font-bold">TCP</span>
                                <span class="text-sm text-gray-600">Transmission Control Protocol</span>
                            </div>
                            <button type="button" @click="newStream.tcp_enabled = !newStream.tcp_enabled" :class="newStream.tcp_enabled ? 'bg-yellow-500' : 'bg-gray-300'" class="relative inline-flex h-6 w-11 items-center rounded-full transition-all">
                        ...
                        <div class="flex items-center justify-between px-4 py-3 hover:bg-gray-50/50 transition-colors">
                            <div class="flex items-center gap-2">
                                <span class="bg-orange-100 text-orange-700 px-2 py-0.5 rounded text-xs font-bold">UDP</span>
                                <span class="text-sm text-gray-600">User Datagram Protocol</span>
                            </div>
                            <button type="button" @click="newStream.udp_enabled = !newStream.udp_enabled" :class="newStream.udp_enabled ? 'bg-yellow-500' : 'bg-gray-300'" class="relative inline-flex h-6 w-11 items-center rounded-full transition-all">
                ...
                <div>
                    <span class="block text-sm font-semibold text-gray-700 mb-2">Target Address <span class="text-red-500">*</span></span>
                    <input v-model="newStream.target" @blur="newStream.target = cleanTarget(newStream.target)" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-gray-700 bg-gray-50/50 focus:outline-none focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500 focus:bg-white transition-all font-mono" placeholder="192.168.1.100:3306">
                    <p class="text-xs text-gray-500 mt-2">Format: hostname:port or ip:port</p>
                </div>
            </div>
            ...
            <div v-if="activeTab === 'ssl'" class="space-y-5">
                ...
                <div class="border border-gray-200 rounded-xl overflow-hidden">
                    <div class="px-4 py-4 flex items-center justify-between bg-gray-50">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <span class="font-semibold text-gray-700">Enable TLS Termination</span>
                                <p class="text-xs text-gray-500">Decrypt incoming TLS traffic</p>
                            </div>
                        </div>
                        <button @click="newStream.ssl = !newStream.ssl" :class="newStream.ssl ? 'bg-green-500' : 'bg-gray-300'" class="relative inline-flex h-6 w-11 items-center rounded-full transition-all">
                            <span :class="newStream.ssl ? 'translate-x-6' : 'translate-x-1'" class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform shadow-sm" />
                        </button>
                    </div>
                </div>
                ...
                <div v-if="newStream.ssl" class="space-y-4">
                    <div>
                        <span class="block text-sm font-semibold text-gray-700 mb-2">SSL Provider</span>
                    ...
                    <div v-if="newStream.ssl_provider === 'custom'">
                        <span class="block text-sm font-semibold text-gray-700 mb-2">Certificate <span class="text-red-500">*</span></span>
